...
type SubscriptionsResult = {
  webhookSubscriptions: {
    nodes: { id: string; topic: string; endpoint: { __typename: string; callbackUrl?: string } }[];
  };
};
...
async function main(): Promise<void> {
  ...
  const { WEBHOOK_SUBSCRIPTIONS_QUERY, WEBHOOK_SUBSCRIPTION_CREATE_MUTATION } = await import(
    '@/lib/shopify/queries/admin'
  );
  ...
  const existing = await adminRequest<SubscriptionsResult>({ query: WEBHOOK_SUBSCRIPTIONS_QUERY });
  ...
  info(`  ${existing.webhookSubscriptions.nodes.length} subscription(s) currently registered on this store`);

  let created = 0;
  for (const topic of REQUIRED_TOPICS) {
    if (registered.has(topic)) {
      console.log(`  ${colors.dim}skip${colors.reset}   ${topic} (already registered)`);
      continue;
    }
    ...
    if (errors.length > 0) {
      console.log(`  ${colors.red}error${colors.reset}  ${topic}: ${errors.map((e) => e.message).join('; ')}`);
      continue;
    }
  ...
  info('Remember: the signing secret must match SHOPIFY_WEBHOOK_SECRET in .env.local');
}
