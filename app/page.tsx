...
import { bestSellersProxy, newArrivals, onSaleProducts } from '@/lib/catalog/recommendations';
import { primaryImage, imageAlt } from '@/lib/utils/image';
import { defaultVariant } from '@/lib/catalog/selectors';

import { ButtonLink } from '@/components/ui/button';
import { ProductGrid } from '@/components/product/product-card';
import { SectionHeading, EmptyState, Price, Badge } from '@/components/ui/primitives';
import { Accordion } from '@/components/ui/accordion';
import { RecentlyViewedSection } from '@/components/product/recently-viewed-section';
import { ChevronRightIcon, GridIcon, RefreshIcon, ShieldIcon, TruckIcon } from '@/components/ui/icons';
  ...
  return (
    <>
          ...
          <div className="grid lg:grid-cols-2">
            <div className="order-2 flex flex-col justify-center gap-6 px-6 py-12 sm:px-10 sm:py-16 lg:order-1 lg:px-14 lg:py-20">
              ...
              <p className="max-w-md text-base leading-relaxed text-ink-muted">
                A tight edit rather than an endless catalogue. Every piece is chosen to earn its
                place, and every order is tracked from checkout to doorstep.
              </p>
            ...
            <div className="relative order-1 aspect-[4/3] lg:order-2 lg:aspect-auto lg:min-h-[540px]">
              {heroImage ? (
                <Image
                  src={heroImage.url}
      ...
      {featuredCollections.length > 0 && (
        <section className="container-page py-14" aria-labelledby="collections-heading">
          <SectionHeading
            eyebrow="Browse"
          ...
          <ul className="grid gap-4 sm:grid-cols-3 sm:gap-5">
            {featuredCollections.map((collection, index) => {
              const image =
                collection.image ??
                primaryImage(products.find((product) => product.collections.some((c) => c.id === collection.id)) ?? products[0]!);
                ...
                <li key={collection.id}>
                  ...
                  >
                    ...
                    <span className="absolute inset-x-0 bottom-0 flex items-end justify-between gap-3 p-5">
                      <span>
                        <span className="block font-display text-xl text-white">{collection.title}</span>
      ...
      {arrivals.length > 0 && (
        <section className="container-page py-14" aria-labelledby="new-heading">
          ...
          <ProductGrid products={arrivals} listName="New arrivals" priorityCount={2} />
        </section>
      )}

      {/* ── Spotlight ────────────────────────────────────────────────── */}
      <section className="container-page py-14" aria-labelledby="spotlight-heading">
        <div className="grid items-center gap-8 rounded-xl bg-surface p-6 sm:p-10 lg:grid-cols-2 lg:gap-14 lg:p-14">
          ...
          <div className="flex flex-col gap-5">
            <p className="text-2xs font-semibold tracking-[0.2em] text-accent uppercase">In focus</p>
      ...
      {bestSellers.length > 0 && (
        <section className="container-page py-14" aria-labelledby="best-heading">
          <SectionHeading
            eyebrow="Popular"
            title="Reaching for again and again"
            action={{ href: '/collections?sort=best-selling', label: 'See all' }}
            className="mb-8"
          />
          <h2 id="best-heading" className="sr-only">
            Popular products
          </h2>
          <ProductGrid products={bestSellers} listName="Best sellers" priorityCount={0} />
        </section>
      )}

      {/* ── Sale ─────────────────────────────────────────────────────── */}
      {sale.length > 0 && (
        <section className="container-page py-14" aria-labelledby="sale-heading">
      ...
      {/* ── Assurance ────────────────────────────────────────────────── */}
      <section className="container-page py-14" aria-labelledby="promise-heading">
        ...
        <ul className="grid gap-6 rounded-xl border border-line bg-surface p-8 sm:grid-cols-3 sm:p-10">
          {[
            { icon: TruckIcon, title: 'Tracked, always', body: 'Follow every order from dispatch to delivery inside your account.' },
            { icon: RefreshIcon, title: 'Returns without friction', body: 'Change your mind? Start a return straight from your order history.' },
            { icon: ShieldIcon, title: 'Checkout you can trust', body: 'Payments handled entirely by Shopify. Your card details never touch us.' },
          ].map((item) => {
            const Icon = item.icon;
            return (
              <li key={item.title} className="flex flex-col gap-2.5">
                <Icon size={26} className="text-accent" />
                <h3 className="font-sans text-base font-medium tracking-normal">{item.title}</h3>
      ...
      <section className="container-page py-14" aria-labelledby="faq-heading">
        <div className="mx-auto max-w-3xl">
          <SectionHeading title="Common questions" align="center" className="mb-8" />
      ...
      <EmptyState
        ...
        action={
          <div className="space-y-3 text-left">
            <p className="text-sm text-ink-muted">Run this from the project root:</p>
            <pre className="overflow-x-auto rounded-md bg-surface-sunken px-4 py-3 text-sm">
              <code>npm run shopify:sync</code>
            </pre>
            <p className="text-xs text-ink-subtle">
              Fill in <code>.env.local</code> from <code>.env.example</code> first.
            </p>
          </div>
        }
      />
