...
export default async function CollectionsPage({ searchParams }: PageProps) {
  ...
  const populated = collections.filter((collection) => collection.productIds.length > 0);
      ...
      <div className="container-page">
        ...
        <header className="max-w-2xl pb-8">
          ...
          <p className="mt-3 text-base text-ink-muted">
            {page.total} product{page.total === 1 ? '' : 's'} across {populated.length} collection
          ...
          <section aria-labelledby="all-collections-heading" className="mb-12">
            ...
            <ul className="hide-scrollbar flex snap-x gap-3 overflow-x-auto pb-2">
              {populated.map((collection) => {
                ...
                return (
                  <li key={collection.id} className="w-44 shrink-0 snap-start sm:w-52">
                    <Link
