...
export function NewsletterForm() {
  ...
  const onSubmit = (event: React.FormEvent) => {
    ...
    router.push(`/account/login?intent=signup&email=${encodeURIComponent(email.trim())}`);
  };

  return (
    <form onSubmit={onSubmit} noValidate className="space-y-2">
      <label htmlFor={inputId} className="block text-2xs font-semibold tracking-[0.16em] text-ink-subtle uppercase">
        Get first access
      </label>
      <div className="flex gap-2">
        <input
          ...
          className="h-11 min-w-0 flex-1 rounded-md border border-line-strong bg-surface px-3.5 text-sm outline-none transition-colors focus-visible:border-ring focus-visible:ring-2 focus-visible:ring-ring/25 aria-[invalid=true]:border-danger"
        />
      ...
      {state === 'error' ? (
        ...
        <p id={`${inputId}-hint`} className="flex items-center gap-1.5 text-xs text-ink-subtle">
          <CheckIcon size={13} />
