...
export function HeroCarousel({
  ...
  /** Desktop/tablet only — replaces the image slides with a looping video when present. No mobile equivalent yet. */
  ...
  React.useEffect(() => {
    if (showVideo || slides.length < 2 || paused || reducedMotion) return;
    const id = window.setInterval(() => {
      setActive((current) => (current + 1) % slides.length);
    }, INTERVAL_MS);
    return () => window.clearInterval(id);
  }, [showVideo, slides.length, paused, reducedMotion]);

  if (slides.length === 0 && mobileSlides.length === 0 && !video) return null;
