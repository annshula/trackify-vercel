...
function assertNoLeakedSecrets(): void {
  ...
  for (const key of Object.keys(process.env)) {
    ...
    if (SECRET_KEYS.some((s) => s === suffix || s.endsWith(suffix))) leaked.push(key);
    if (/(TOKEN|SECRET|PASSWORD|PRIVATE_KEY|ADMIN_API_KEY)$/.test(suffix)) leaked.push(key);
  }
  if (leaked.length > 0) {
    throw new Error(
...
function buildEnv() {
  const storeDomain = normalizeDomain(
    required('SHOPIFY_STORE_DOMAIN', 'Set it to your permanent myshopify domain (no protocol).'),
  );
  ...
  return {
    ...
    customerAccount: {
      ...
      clientSecret: optional('SHOPIFY_CUSTOMER_ACCOUNT_CLIENT_SECRET'),
    },
...
export function customerAccountConfig() {
  const env = serverEnv();
  const { shopId, clientId, clientSecret } = env.customerAccount;
