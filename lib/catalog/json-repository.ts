...
export class JsonProductRepository implements ProductRepository {
  ...
  #setCatalog(catalog: Catalog): void {
    // Reads go through the hydrated copy: the document on disk may have been
    // written by an older deployment whose records lack fields added since.
    // The write paths still persist exactly what they were given.
    const hydrated = hydrateCatalog(catalog);
    this.#catalog = hydrated;
    this.#byHandle = new Map(
      hydrated.products.map((product) => [product.handle, product]),
    );
    this.#byId = new Map(
      hydrated.products.map((product) => [product.id, product]),
    );
    this.#collectionsByHandle = new Map(
      hydrated.collections.map((collection) => [collection.handle, collection]),
    );
    this.#availableProducts = hydrated.products.filter(isPurchasable);
    this.#loading = null;
  }
  ...
  async getProductsByCollection(
    ...
    return paginate(
      applyFilters(members, query),
      query,
      buildFacets(members, query),
    );
  }
  ...
  async searchProducts(term: string, query: ProductQuery = {}) {
    ...
    return {
      hits: page.products.map<SearchHit>(
        (product) =>
          hitById.get(product.id) ?? { product, score: 0, matchedOn: [] },
      ),
      total: page.total,
      facets: page.facets,
    };
...
export class JsonRedirectRepository implements RedirectRepository {
  ...
  async #load(): Promise<RedirectMap> {
    this.#map ??=
      (await readJsonFile<RedirectMap>(REDIRECTS_PATH)) ?? EMPTY_REDIRECTS;
    return this.#map;
  }
...
export function applyFilters(
        ...
        product.variants.some((variant) =>
          variant.selectedOptions.some(
            (option) =>
              ...
              wanted.has(option.value.toLowerCase()),
          ),
        ),
      );
    }
  ...
  if (typeof filters.minPrice === "number") {
    result = result.filter(
      (product) => product.priceRange.max >= filters.minPrice!,
    );
  }
  if (typeof filters.maxPrice === "number") {
    result = result.filter(
      (product) => product.priceRange.min <= filters.maxPrice!,
    );
  }
