...
vi.mock("@/lib/catalog/storage", async () => {
  const actual = await vi.importActual<typeof import("@/lib/catalog/storage")>(
    "@/lib/catalog/storage",
  );
  return { ...actual, CATALOG_PATH, REDIRECTS_PATH, DATA_DIR: TEMP_DIR };
});

const { JsonProductRepository, JsonRedirectRepository } =
  await import("@/lib/catalog/json-repository");
const { makeCatalog, makeCollection, makeProduct, makeVariant } =
...
describe("JsonProductRepository", () => {
  ...
  it("reads a document written before specs and feature highlights existed", async () => {
    // A seed file or Blob left behind by an older deployment has no such keys.
    // Reading one straight through threw on `product.specs.length` and failed
    // the build on the first product page it prerendered.
    const legacy: Partial<CatalogProduct> = makeProduct({
      handle: "legacy-shape",
    });
    delete legacy.specs;
    delete legacy.featureHighlights;
    await fs.writeFile(
      CATALOG_PATH,
      JSON.stringify(makeCatalog({ products: [legacy as CatalogProduct] })),
      "utf8",
    );

    const product = await repo.getProductByHandle("legacy-shape");
  ...
  it("hides an unpublished product from the storefront", async () => {
    await repo.replaceCatalog(
      makeCatalog({
        products: [makeProduct({ handle: "draft-item", status: "DRAFT" })],
      }),
    );
    expect(await repo.getProductByHandle("draft-item")).toBeNull();
    // Diagnostics can still see it.
    expect(
      await repo.getAllProducts({ includeUnavailable: true }),
    ).toHaveLength(1);
  });
  ...
  it("hides a product that is active but not published online", async () => {
    await repo.replaceCatalog(
      makeCatalog({
        products: [makeProduct({ handle: "hidden", publishedOnline: false })],
      }),
    );
  ...
  it("deletes a product and cleans its collection membership", async () => {
    ...
    await repo.replaceCatalog(
      makeCatalog({ products: [product], collections: [collection] }),
    );
  ...
  it("reports false when deleting something that is already gone", async () => {
    ...
    expect(
      await repo.deleteProduct("gid://shopify/Product/does-not-exist"),
    ).toBe(false);
  });
  ...
  it("paginates a collection listing", async () => {
    ...
    await repo.replaceCatalog(
      makeCatalog({ products, collections: [collection] }),
    );

    const page = await repo.getProductsByCollection("big", {
      page: 2,
      perPage: 12,
    });
  ...
  it("clamps a page beyond the end rather than returning nothing", async () => {
    ...
    const page = await repo.getProductsByCollection("small", {
      page: 99,
      perPage: 12,
    });
...
describe("JsonRedirectRepository", () => {
  ...
  it("keeps product and collection namespaces separate", async () => {
    ...
    expect(await redirects.resolveCollection("old-collection")).toBe(
      "new-collection",
    );
  });
});
